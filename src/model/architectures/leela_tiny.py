...
class LeelaChessNetTiny(nn.Module):
    ...
    Architecture:
    - Reduced channels: 128 (instead of 256)
    - Fewer residual blocks: 4 (instead of 6+)
    - Smaller value head: 128 -> 64 -> 1
    - Optimized for FP16/INT8 quantization
    
    Target size: <5MB (FP16) or <2.5MB (INT8)
    """
    
    def __init__(self, num_residual_blocks: int = 4, num_moves: int = 1968):
        """
        ...
        # Input: 18 channels
        # Reduced initial convolution (128 channels instead of 256)
        ...
        self.value_fc1 = nn.Linear(8 * 8, 128)  # Reduced from 256
        self.value_fc2 = nn.Linear(128, 64)     # Additional layer for capacity
