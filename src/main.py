        ...
        if current_branch == 'distilled-model':
            ...
            if not model_path:
                ...
                if base_opening_models:
                    base_tiny_models.sort(key=lambda x: os.path.getmtime(os.path.join(model_dir, x)), reverse=True)
                    model_path = os.path.join(model_dir, base_tiny_models[0])
                    print(f"✓ [BRANCH: base-advanced-opening] Using BASE_ADVANCED with openings: {base_tiny_models[0]}")
            ...
            if not model_path:
                ...
                if tiny_models:
                    tiny_models.sort(key=lambda x: os.path.getmtime(os.path.join(model_dir, x)), reverse=True)
                    model_path = os.path.join(model_dir, tiny_models[0])
                    print(f"✓ [BRANCH: distilled-model] Using opening-trained FINAL_BEST_MODEL: {tiny_models[0]}")
            ...
            if not model_path:
                ...
                if tiny_models:
                    tiny_models.sort(key=lambda x: os.path.getmtime(os.path.join(model_dir, x)), reverse=True)
                    model_path = os.path.join(model_dir, tiny_models[0])
                    print(f"✓ [BRANCH: distilled-model] Using latest FINAL_BEST_MODEL: {tiny_models[0]}")
            ...
            # Priority 2: Look for FINAL_BEST_MODEL
            if not model_path:
                tiny_models = [f for f in os.listdir(model_dir) if (f.startswith('tiny_model_') or f.startswith('DISTILLED_')) and f.endswith('.pth')]
                if tiny_models:
                    tiny_models.sort(key=lambda x: os.path.getmtime(os.path.join(model_dir, x)), reverse=True)
                    model_path = os.path.join(model_dir, tiny_models[0])
                    print(f"✓ [BRANCH: main] Using FINAL_BEST_MODEL: {tiny_models[0]}")
                ...
                if downloaded_path and os.path.exists(downloaded_path):
                    ...
                    if current_branch == 'distilled-model':
                        ...
                        if tiny_models:
                            tiny_models.sort(key=lambda x: os.path.getmtime(os.path.join(model_dir, x)), reverse=True)
                            model_path = os.path.join(model_dir, tiny_models[0])
                            print(f"✓ [BRANCH: distilled-model] Found opening-trained FINAL_BEST_MODEL after download: {tiny_models[0]}")
                        else:
                            tiny_models = [f for f in os.listdir(model_dir) if (f.startswith('tiny_model_') or f.startswith('DISTILLED_')) and f.endswith('.pth')]
                            if tiny_models:
                                tiny_models.sort(key=lambda x: os.path.getmtime(os.path.join(model_dir, x)), reverse=True)
                                model_path = os.path.join(model_dir, tiny_models[0])
                                print(f"✓ [BRANCH: distilled-model] Found FINAL_BEST_MODEL after download: {tiny_models[0]}")
                    elif current_branch == 'leela':
                        ...
                        if leela_models:
                            ...
                            if tiny_models:
                                tiny_models.sort(key=lambda x: os.path.getmtime(os.path.join(model_dir, x)), reverse=True)
                                model_path = os.path.join(model_dir, tiny_models[0])
                                print(f"✓ [BRANCH: main] Found FINAL_BEST_MODEL after download: {tiny_models[0]}")
                    ...
                    if os.path.exists(chess_bot_model_dir):
                        ...
                        if os.path.exists(model_dir):
                            ...
                            if tiny_models:
                                tiny_models.sort(key=lambda x: os.path.getmtime(os.path.join(model_dir, x)), reverse=True)
                                model_path = os.path.join(model_dir, tiny_models[0])
                                print(f"✓ [BRANCH: distilled-model] Using fallback FINAL_BEST_MODEL: {tiny_models[0]}")
                            else:
                                tiny_models = [f for f in os.listdir(model_dir) if (f.startswith('tiny_model_') or f.startswith('DISTILLED_')) and f.endswith('.pth')]
                                if tiny_models:
                                    tiny_models.sort(key=lambda x: os.path.getmtime(os.path.join(model_dir, x)), reverse=True)
