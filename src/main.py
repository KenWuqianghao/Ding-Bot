...
def initialize_engine():
    ...
    # This file is for the leela branch - prioritize leela_best models
    BRANCH_NAME = 'leela'
    hf_download_priority = ['leela_best']  # Will match both openings and regular leela_best
    ...
    if os.path.exists(model_dir):
        ...
        if not model_path:
            ...
            if leela_best_models:
                ...
                print(f"✓ [BRANCH: leela] Using leela_best: {leela_best_models[0]}")
                print(f"  This model was fine-tuned on opening positions for better opening play!")
        # Priority 2: Look for latest leela_best (current training run)
        if not model_path:
            leela_best_models = [f for f in os.listdir(model_dir) if f.startswith('leela_best_') and f.endswith('.pth')]
            if leela_best_models:
                leela_best_models.sort(key=lambda x: os.path.getmtime(os.path.join(model_dir, x)), reverse=True)
                model_path = os.path.join(model_dir, leela_best_models[0])
                print(f"✓ [BRANCH: leela] Using latest leela_best: {leela_best_models[0]}")
        # Priority 3: Fallback to most recent .pth file (only if no leela_best found)
        if not model_path:
            ...
            try:
                ...
                if downloaded_path and os.path.exists(downloaded_path):
                    ...
                    if leela_best_models:
                        leela_best_models.sort(key=lambda x: os.path.getmtime(os.path.join(model_dir, x)), reverse=True)
                        model_path = os.path.join(model_dir, leela_best_models[0])
                        print(f"✓ [BRANCH: leela] Found leela_best after download: {leela_best_models[0]}")
                    else:
                        leela_best_models = [f for f in os.listdir(model_dir) if f.startswith('leela_best_') and f.endswith('.pth')]
                        if leela_best_models:
                            leela_best_models.sort(key=lambda x: os.path.getmtime(os.path.join(model_dir, x)), reverse=True)
                    ...
                    if not model_path:
                        # Use downloaded model only if no leela_best found
                    ...
                    if os.path.exists(chess_bot_model_dir):
                        ...
                        if os.path.exists(model_dir):
                            ...
                            if leela_best_models:
                                leela_best_models.sort(key=lambda x: os.path.getmtime(os.path.join(model_dir, x)), reverse=True)
                                model_path = os.path.join(model_dir, leela_best_models[0])
                                print(f"✓ [BRANCH: leela] Using fallback leela_best: {leela_best_models[0]}")
                            else:
                                leela_best_models = [f for f in os.listdir(model_dir) if f.startswith('leela_best_') and f.endswith('.pth')]
                                if leela_best_models:
                                    leela_best_models.sort(key=lambda x: os.path.getmtime(os.path.join(model_dir, x)), reverse=True)
    ...
    engine = ChessEngine(
        ...
        search_depth=2,  # Reduced to 2 to prevent timeouts (adaptive depth handles time limits)
